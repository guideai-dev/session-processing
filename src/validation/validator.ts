...
export function validateJSONL(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    result.parsedLines++
    // parseResult.success is true, so message must exist
...
export function generateValidationReport(
  ...
  lines.push(`Valid Messages: ${colors.green(String(result.validMessages))}/${result.parsedLines}`)

  if (result.invalidJSONLines.length > 0) {
    lines.push(`Invalid JSON Lines: ${colors.red(String(result.invalidJSONLines.length))}`)
  }
  ...
  if (result.errors.length > 0) {
    ...
    if (!verbose && result.errors.length > 10) {
      lines.push(`  ${colors.yellow(`... and ${result.errors.length - 10} more errors`)}`)
      lines.push(`  ${colors.cyan('(use --verbose to see all errors)')}`)
    }
  }
  ...
  if (result.sessionResult && result.sessionResult.warnings.length > 0) {
    lines.push('')
    lines.push(colors.bold(colors.yellow(`Warnings (${result.sessionResult.warnings.length}):`)))

    for (const warning of result.sessionResult.warnings.slice(0, verbose ? undefined : 5)) {
      lines.push(`  ${colors.yellow('⚠')} Line ${warning.line || '?'}: ${warning.message}`)
  ...
  // Tool chain issues
  if (result.sessionResult && result.sessionResult.toolChainIssues.length > 0) {
    lines.push('')
    lines.push(
      colors.bold(
        colors.yellow(`Tool Chain Issues (${result.sessionResult.toolChainIssues.length}):`)
      )
    )

    for (const issue of result.sessionResult.toolChainIssues.slice(0, verbose ? undefined : 5)) {
