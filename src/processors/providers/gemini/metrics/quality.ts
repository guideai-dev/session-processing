...
export class GeminiQualityProcessor extends BaseMetricProcessor {
  ...
  async process(session: ParsedSession): Promise<SessionMetricsData> {
    ...
    const avgMessageLength = assistantMessages.length > 0
    ...
    const thinkingDepthScore = Math.min(100,
      thinkingAnalysis.avgThoughtsPerMessage * 10
    )
    ...
    const overallQuality = Math.min(100,
      thinkingDepthScore * 0.30 +
      detailScore * 0.25 +
  ...
  private estimateTaskSuccess(
    ...
    if (lastUserMessage) {
      ...
      if (content.includes('thank') || content.includes('perfect') ||
          content.includes('great') || content.includes('done')) {
        return 100
      }

      // Negative signals
      if (content.includes('error') || content.includes('not working') ||
