...
export class CopilotPerformanceProcessor extends BaseMetricProcessor {
  ...
  async process(session: ParsedSession): Promise<PerformanceMetrics> {
    ...
    return {
      ...
      metadata: {
        ...
        improvement_tips: this.generateImprovementTips(averageResponseTime, taskCompletionTime, averageToolExecutionTime),
  ...
  private generateImprovementTips(responseLatency: number, completionTime: number, toolExecutionTime: number): string[] {
    const tips: string[] = []
    ...
    if (toolExecutionTime > 5000) { // > 5 seconds average tool execution
