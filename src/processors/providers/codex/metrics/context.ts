...
/**
 * Codex Context Management Processor
 *
 ...
 *
 * Note: Codex uses the same token schema as Claude Code.
 */
export class CodexContextProcessor extends BaseMetricProcessor {
  ...
  // Codex uses Claude Sonnet 4.5 context window (same as Claude Code)
   ...
   *
   * Follows the same implementation as Claude Code:
   ...
   *
   * Compaction events are detected from system entries with subtype 'compact'
   * and metadata indicating a compaction trigger.
   */
  private detectCompactEvents(session: ParsedSession): {
    ...
    session.messages.forEach((message, index) => {
      // Check for system messages with compact subtype
      if (message.type === 'system' && message.metadata?.subtype === 'compact') {
        compactSteps.push(index + 1) // 1-based indexing
      }
      // Also check for isCompactSummary flag on user messages
      else if (message.type === 'user' && message.metadata?.isCompactSummary === true) {
        compactSteps.push(index + 1)
      }
    })
