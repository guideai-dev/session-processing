...
export class CodexUsageProcessor extends BaseMetricProcessor {
  ...
  async process(session: ParsedSession): Promise<UsageMetrics> {
    ...
    const readTools = ['read_file', 'view_file', 'search_files', 'list_directory'] // Tools that consume information
    const writeTools = ['edit_file', 'create_file', 'str_replace_editor'] // Tools that produce changes
