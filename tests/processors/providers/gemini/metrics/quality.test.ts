...
import { GeminiQualityProcessor } from '../../../../../src/processors/providers/gemini/metrics/quality.js'
...
describe('GeminiQualityProcessor', () => {
	const processor = new GeminiQualityProcessor()
	...
	describe('basic metrics with real session', () => {
		...
		it('should include metadata if present', async () => {
			...
			if (metrics.metadata) {
				if (metrics.metadata.thinking_depth_score !== undefined) {
					expect(metrics.metadata.thinking_depth_score).toBeGreaterThanOrEqual(0)
				}
			}
		})
	})
